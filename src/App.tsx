...
function AppBody({ preloadedData }: AppBodyProps) {
  ...
  const handleLoginSuccess = (rawUser: any) => {
    const staffMember: StaffMember = {
      ...
      isPlatformAdmin: !!rawUser.is_platform_admin,
    };
    login(staffMember);
  ...
  const handleAddGuest = async (newGuest: Guest) => {
    ...
    try {
      const result = await addGuestToDB({
        ...
        advance_payment_mode: (newGuest as any).advancePaymentMode || 'Cash',
        advance_payment_proof_base64: (newGuest as any).advancePaymentProofBase64 || undefined,
        pending_payment_mode: (newGuest as any).pendingPaymentMode || 'Cash',
        pending_payment_proof_base64: (newGuest as any).pendingPaymentProofBase64 || undefined,
