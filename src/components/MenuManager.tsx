    ...
    <div className="space-y-5">
      ...
      {activeSubTab === 'nav_menu' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs p-4 sm:p-6 space-y-5">
          {/* Header & Drag Banner Info */}
          <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-slate-100 gap-3">
            <div>
              <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-emerald-600" /> Main Menu Drag & Drop Hierarchy & RBAC Permissions
              </h3>
              <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1.5">
                <Info className="w-3.5 h-3.5 text-blue-600 inline shrink-0" />
                <span>
                  <strong>Drag & Drop:</strong> Grab the <GripVertical className="w-3.5 h-3.5 inline text-slate-400" /> grip handles to reorder any menu item. Changes reflect live across the application sidebar!
                </span>
              </p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <span className="text-[11px] font-bold bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-lg text-emerald-800">
                Active User Role: <strong>{activeRole}</strong>
              </span>
              <button
                onClick={handleOpenAddNavModal}
                className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs px-3.5 py-1.5 rounded-lg flex items-center gap-1.5 shadow-2xs transition-all cursor-pointer"
              >
          ...
          <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-3">
            <div className="flex flex-col sm:flex-row items-center gap-2.5 w-full md:w-auto flex-1">
              ...
              <div className="relative flex-1 w-full sm:w-72">
                ...
                <input
                  ...
                  className="bg-white border border-slate-300 text-slate-900 text-xs rounded-lg focus:ring-2 focus:ring-emerald-500 block w-full pl-9 p-2"
                />
              </div>
              ...
              <select
                ...
                className="bg-white border border-slate-300 text-slate-900 text-xs rounded-lg focus:ring-2 focus:ring-emerald-500 p-2 font-semibold w-full sm:w-auto"
              >
            ...
            <div className="text-xs text-slate-500 font-medium">
              Showing <strong>{filteredNavItems.length}</strong> of <strong>{navItems.length}</strong> menu items
            </div>
          </div>

          {/* DRAG AND DROP MENU LIST */}
          <div className="space-y-3">
            {filteredNavItems.length === 0 ? (
              <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-xl">
                <Grid className="w-10 h-10 text-slate-300 mx-auto mb-2" />
                <p className="text-slate-600 font-bold text-sm">No menu items match your search filter</p>
                <button
                  onClick={() => {
                    setNavSearch('');
                    setSelectedNavCategory('All Categories');
                  }}
                  ...
                  <div
                    ...
                    className={`group p-4 rounded-xl border transition-all flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 cursor-grab active:cursor-grabbing ${
                      ...
                      isBeingDragged
                        ...
                        : isDragTarget
                        ? 'border-2 border-blue-600 bg-blue-50/80 shadow-md ring-2 ring-blue-200'
                        : nav.isVisible
                        ? 'bg-slate-50/90 border-slate-200 hover:border-slate-300 hover:bg-white shadow-2xs'
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 w-full lg:w-auto">
                      ...
                      <div
                        ...
                        title="Click and drag to reorder menu item"
                      >
                        <GripVertical className="w-5 h-5" />
                      </div>

                      {/* Order Up / Down Arrows (Fallback) */}
                      <div className="flex flex-col gap-0.5 shrink-0">
                        <button
                          disabled={index === 0}
                          onClick={() => handleMoveNavItem(index, 'up')}
                          className="p-1 rounded bg-white hover:bg-slate-200 text-slate-600 disabled:opacity-30 border border-slate-200 cursor-pointer"
                          title="Move Up"
                        >
                      ...
                      <button
                        onClick={() => setIconPickerTargetId(nav.id)}
                        className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-700 hover:border-emerald-500 hover:text-emerald-600 transition-all shadow-2xs relative group/icon cursor-pointer shrink-0"
                        title="Click to Change Menu Icon"
                      >
                        <DisplayIcon className="w-5 h-5" />
                        <span className="absolute -top-1 -right-1 bg-emerald-600 text-white rounded-full p-0.5 opacity-0 group-hover/icon:opacity-100 transition-opacity">
                      ...
                      <div>
                        ...
                        <span className="text-[11px] text-slate-400 block mt-0.5">
                          Order Sequence: #{nav.order} {nav.uniqueKey ? `| Route Key: ${nav.uniqueKey}` : ''}
                        </span>
                      </div>
                    </div>

                    {/* Middle RBAC Role Checkboxes */}
                      ...
                      <div className="flex flex-wrap items-center gap-1.5">
                        {Array.from(new Set([...DEFAULT_SYSTEM_ROLES, ...navItems.flatMap((i) => i.roles || [])])).map((role) => {
                          ...
                          return (
                            <button
                              key={role}
                              onClick={() => handleToggleRolePermission(nav.id, role)}
                    ...
                    <div className="flex items-center gap-2 shrink-0 self-end lg:self-center">
                      <button
                        onClick={() => handleToggleNavVisibility(nav.id)}
                        ...
                        title={nav.isVisible ? 'Visible in Navigation' : 'Hidden from Navigation'}
                      >
                        {nav.isVisible ? (
                        ...
                        <span>{nav.isVisible ? 'Visible' : 'Hidden'}</span>
                      </button>

                      <button
                        onClick={() => handleOpenEditNavModal(nav)}
