...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({
  ...
  // flowbite-react's <Tabs> only reads each TabItem's `active` prop once, at
  // its own mount, to seed which tab starts selected - it does NOT re-read it
  // on later renders, so external changes to `activeTab` (the sidebar-driven
  // effect above, or the kitchenModuleEnabled guard below) would silently
  // fail to move the visible tab even though our own `activeTab` state is
  // correct. Tabs exposes an imperative `setActiveTab(index)` via ref for
  // exactly this - push our state into it whenever it changes, regardless of
  // source (a user click already updates both sides via onActiveTabChange
  // below, so this just re-applies the same index in that case - harmless).
  const tabsRef = useRef<TabsRef>(null);
  useEffect(() => {
    const tabOrder = kitchenModuleEnabled
  ...
  return (
    <div className="analytics-dashboard-container space-y-6 text-slate-800 dark:text-slate-200">
      ...
      {/* Report-Type Switcher - converted to flowbite-react Tabs/TabItem
      ...
      >
      {/* TAB 1: OVERVIEW */}
      <TabItem active={activeTab === 'overview'} title={t('overview_tab_label', 'Overview')} icon={TrendingUp}>
        <div className="analytics-overview space-y-6">
          <div className="analytics-overview__card bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-md space-y-4">
      ...
      {/* TAB 2: BOOKINGS */}
      <TabItem active={activeTab === 'bookings'} title={t('bookings_tab_label', 'Bookings')} icon={Calendar}>
        <div className="analytics-bookings space-y-6">
          <div className="analytics-bookings__card bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-md space-y-4">
          ...
          <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-md space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <span className="text-xs font-bold px-2.5 py-1 rounded-md bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
      ...
      <TabItem active={activeTab === 'pace'} title={t('pace_tab_label', 'Pace')} icon={CalendarClock}>
        <div className="analytics-pace space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {paceBuckets.map((b) => (
              <div key={b.key} className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-md">
      ...
      {kitchenModuleEnabled && (
      <TabItem active={activeTab === 'kitchen'} title={t('kitchen_food_pos_tab_label', 'Kitchen & Food POS')} icon={Utensils}>
        <div className="analytics-kitchen space-y-6">
          <div className="analytics-kitchen__card bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-md space-y-5">
            ...
            {/* KPI Summary Rows */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-4 bg-emerald-50 dark:bg-emerald-950/30 rounded-lg border border-emerald-200 dark:border-emerald-800 shadow-md">
              ...
              <div className="p-4 bg-amber-50 dark:bg-amber-950/30 rounded-lg border border-amber-200 dark:border-amber-800 shadow-md">
              ...
              <div className={`p-4 rounded-lg border shadow-md ${kitchenNetProfit >= 0 ? 'bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800' : 'bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800'}`}>
            ...
            <div className="bg-slate-50/20 dark:bg-slate-900/10 p-4 sm:p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-md space-y-4 mt-6">
      ...
      {/* TAB 5: EXPENSES */}
      <TabItem active={activeTab === 'expenses'} title={t('expenses_tab_label', 'Expenses')} icon={ShoppingBag}>
        <div className="analytics-expenses space-y-6">
          <div className="analytics-expenses__card bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-md space-y-4">
      ...
      {/* TAB 6: PROFIT & LOSS */}
      <TabItem active={activeTab === 'profit_loss'} title={t('pnl_tab_label', 'P&L')} icon={BarChart3}>
        <div className="analytics-profit-loss space-y-6">
          <div className="analytics-profit-loss__card bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-md space-y-4">
      ...
      <TabItem active={activeTab === 'fluctuations'} title={t('fluctuations_tab_label', 'Fluctuations')} icon={Activity}>
