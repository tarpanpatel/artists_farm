...
export interface AirbnbIconProps extends React.SVGProps<SVGSVGElement> {
  className?: string;
}

/**
 * Authentic Airbnb Brand "Bélo" Icon (SVG, official coral #FF5A5F mark) -
 * added 3 Sep 2026 so an OTA booking is instantly recognizable by its real
 * channel logo instead of just the text "AirBNB" - used wherever a guest's
 * `otaSource`/`otaSourceLabel` is "airbnb" (calendar capsules, booking
 * details, channel manager pages). Same reusable-icon-component pattern as
 * TelegramIcon.tsx/WhatsappIcon.tsx, except this renders in its fixed brand
 * color rather than `currentColor` - Airbnb's coral IS the mark's identity,
 * same reason a payment app never recolors the Visa/Mastercard logo.
 */
export const AirbnbIcon: React.FC<AirbnbIconProps> = ({ className = 'w-4 h-4', ...props }) => (
  <svg
    viewBox="0 0 44 48"
    aria-hidden="true"
    className={className}
