...
export const Tooltip: React.FC<TooltipProps> = ({ content, children, position = 'top', className = '' }) => {
  ...
  return (
    <span className={`relative inline-flex group ${className}`}>
      ...
      <span
        role="tooltip"
        className={`absolute z-[99999] ${positionClasses[position]} w-max max-w-xs p-2 bg-slate-900 dark:bg-slate-800 text-white text-[11px] font-normal leading-snug rounded-xl shadow-2xl opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-200 pointer-events-none text-center border border-slate-700/60`}
      >
        {content}
