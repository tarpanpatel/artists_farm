      ...
      >
        <div className="min-w-max">
          <div className="flex border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80">
            ...
            <div className="w-24 min-w-24 sticky left-0 z-20 bg-slate-50 dark:bg-slate-800 px-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 border-r border-slate-200 dark:border-slate-700 flex items-center py-2">
              Room
            </div>
                    ...
                    <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
                      {timelineLanesInfo.map((info, idx) => {
                        ...
                        if (info.item.kind === 'ota') {
                          ...
                          const BlockOtaIcon = getOtaIcon((otaItem as any).source || otaItem.label);
                          return (
                            <Popover
                              key={otaPopoverKey}
                              trigger="click"
                              ...
                              title={
                                <h4 className="font-semibold text-gray-900 dark:text-white text-xs truncate flex items-center gap-1.5">
                                  ...
                                  <span className="truncate">{otaItem.label}</span>
                                </h4>
                              }
                              content={
                        ...
                        const guestName = guest.guestName?.trim() || (guest as any).name?.trim();
                        const backupName = (guest as any).otaSourceLabel || (guest as any).otaSource || t('booked_guest_fallback', 'Booked');
                              ...
                              <div className="flex items-center justify-between gap-2">
                                <h4 className="font-semibold text-gray-900 dark:text-white text-xs truncate flex items-center gap-1.5">
                                  {isOtaBooking && (
                                    <span className="inline-flex items-center justify-center w-4 h-4 rounded-[4px] bg-white/90 shadow-2xs shrink-0 p-0.5">
                                      {OtaIcon ? (
                                        <OtaIcon className="w-3.5 h-3.5 shrink-0" />
                                      ) : (
