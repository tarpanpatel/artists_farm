      ...
      cell: (row: Guest) => (
        <div className="flex flex-col py-2">
          <div className="text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
            <span className="text-[10px] uppercase text-emerald-600 bg-emerald-50 dark:bg-emerald-950/20 px-1 py-0.5 rounded-sm">IN</span>
            <span>{row.checkinDate}</span>
          </div>
          <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1.5 mt-1">
            <span className="text-[10px] uppercase text-rose-600 bg-rose-50 dark:bg-rose-950/20 px-1 py-0.5 rounded-sm">OUT</span>
    ...
    },
    {
      ...
      cell: (row: Guest) => (
        <div className="flex items-center gap-1.5 font-semibold text-slate-800 dark:text-slate-200">
          ...
          <span>{row.roomNumber || 'Unassigned'}</span>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: (row: Guest) => {
        if (!row.isForeignGuest) {
          return <span className="text-slate-400 dark:text-slate-600 text-xs">N/A (Indian National)</span>;
        }
    ...
    <div className="space-y-6 text-xs text-slate-800 dark:text-slate-200">
      ...
      <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
            ...
            <span>Guest Registration Archive</span>
          </h2>
          <p className="text-xs text-slate-500 mt-1">
            Browse complete history of current, upcoming, and past guest bookings, stays, and ledger records.
          </p>
        </div>
        ...
        >
          ...
          <span>Export filtered list (CSV)</span>
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xs flex flex-col lg:flex-row items-center gap-4">
        ...
        <div className="relative w-full lg:w-96">
          ...
          <input
            type="text"
            placeholder="Search by guest name, phone, room..."
            value={searchTerm}
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto lg:ml-auto">
          ...
          <div className="flex items-center gap-1.5">
            ...
            >
              ...
              <option value="Booked">Reserved only</option>
            </select>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            >
              ...
              <option value="Foreigner">Foreign Nationals (C-Form required)</option>
              <option value="Indian">Indian Nationals</option>
            </select>
          </div>
        </div>
      </div>
