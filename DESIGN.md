...
## 18. Field Label & Section Heading Typography Standard
- **One Size, One Weight, Per Role - No Ad-Hoc Mixing**: Within any single form/modal, every input field `<label>` must share the exact same size and weight - `text-[11px] font-semibold uppercase` (paired with `text-slate-600 dark:text-slate-400` or `text-slate-500` for muted fields). Do not mix `text-[10px]` and `text-[11px]`, or `font-semibold` and `font-extrabold`/`font-bold`, across labels that play the same role in the same view - found and fixed 19 Aug 2026 in `ReceiptEditModal.tsx`, where "Received By (Booking)", "Select Dish / Item", "Quantity", and "Pending Received By" were `text-[10px]` while every other field label in the same modal was `text-[11px]`, and "Split Distribution Matrix" was `font-extrabold` while its four sibling section headings were `font-semibold`.
- **Section Headings Are a Separate, Also-Internally-Consistent Role**: A section heading (e.g. *Food Orders & Incidentals Log*, *Final Checkout Split Settlement*) is visually distinct from a field label by design (see Section 6's `font-extrabold text-sm` standard for page-level section headers) - it is fine for section headings to look different from field labels. What's never fine is inconsistency *within* the same role: every section heading in a given view must share one identical size/weight, and every field label in that view must share a separate, single identical size/weight.
