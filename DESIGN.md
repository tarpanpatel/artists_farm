...
All action modals, creation forms, and secondary management dialogs across the site (e.g. `New Service Request`, item configurations, edit sheets) - **except any dialog that can be opened while another drawer is already on screen, which is a centered `<Modal>` instead; see the MANDATORY nested-dialog rule below**:
...
**MANDATORY - nested dialogs never stack a second Drawer (25 Aug 2026, explicit user report + rule request; re-emphasised 31 Aug 2026)**: this is a *separate* rule from the confirmation/alert exception above and applies regardless of content length or field count - a real multi-field form is included. If a dialog is opened while ANOTHER drawer is already open (i.e. it's a step/sub-action reached from inside that drawer, not a top-level entry point of its own), it must be a centered `<Modal>`, never a second right-side `<Drawer>` stacked on top. Two same-edge slide-over drawers on screen at once reads as "the first one closed and a different one opened," not "a step within the same flow" - and closing the inner one has repeatedly been coded (wrongly) to also close the outer one "since you're back at a blank drawer edge anyway," which is its own separate bug class this rule prevents by construction.
  - **General principle (31 Aug 2026, explicit user instruction: "always avoid back-to-back drawer opening unless very necessary")**: opening a drawer directly from within another drawer is confusing by default - treat it as a smell, not a pattern. Before building one, exhaust the alternatives: fold the sub-action into the parent drawer inline, make it a centered `<Modal>` on top, or route to it as a full screen/tab instead. A second stacked drawer is only acceptable when there is a genuine, unavoidable reason no other shape fits - and that reason gets written down next to the code.
  - If a dialog has BOTH a top-level entry point (e.g. a page-header button) AND an in-drawer entry point, the in-drawer one wins the decision: build it as a `<Modal>` so the same component satisfies both. Give it `z-70` (above the drawer's `z-58`), the same header/scroll-body/footer structure as the drawers here, and `dismissible` + `popup`.
  ...
  - Found live 31 Aug 2026: `ServiceRequestsManagement.tsx`'s "Manage Custom Services" was a second right-side Drawer, opened both from the page header AND from a link inside the open "New Service Request" drawer. Converted to a `z-70` `<Modal>`.
