...
- **Calendar bar conventions for OTA bookings (added 28 Aug 2026, explicit request)** - applies to both `TodayOverview.tsx` (multi-room overview) and `OperationalDashboard.tsx` (per-room calendar); both must stay in sync since they render the same concepts:
  ...
  - **A converted OTA booking's bar must show the guest's first name followed by the OTA source in parentheses** - e.g. `Gaurav (Booking.com)`, not just the guest's name with an icon. Read `guest.otaSourceLabel || guest.otaSource` (already fetched/typed on the Guest object, see `src/types.ts` and `fetchGuestsFromDB()` in `api.ts` - no backend change needed) rather than only showing the generic `Globe` icon as before.
