...
## 🌓 Dark Mode — Intentionally Disabled (added 28 Aug 2026)

**Dark mode is deliberately OFF app-wide - do not re-enable it without the user explicitly asking again.** `src/index.css` configures Tailwind's `dark:` variant as CLASS-based (`@custom-variant dark (&:where(.dark, .dark *));`), not the default media-query strategy - which means every `dark:` utility class across the whole app only ever applies if something adds a `.dark` class to `<html>`. As of 28 Aug 2026, **nothing does** - a same-day investigation found this had *always* been the case (confirmed via a full `grep` of `src/`: no `classList.add('dark')`/`matchMedia` wiring exists anywhere; `themeService.ts`'s `enableDarkModeToggle` flag is declared and defaulted but never read). A fix was built and deployed the same day (an `index.html` inline bootstrap script + a `main.tsx` `matchMedia` listener, both since removed) - then the user explicitly said **"remove dark mode from this app for now as I will never need it"**, reversing that fix within the hour.

- **The thousands of existing `dark:` utility classes throughout the codebase are left as-is, untouched** - removing them would be a massive, invasive, hard-to-reverse sweep across nearly every component for a purely cosmetic no-op (they're already inert with no `.dark`-toggle mechanism in place). Leave them; don't "clean them up" either.
- **`index.html`'s `#initial-loader`** (the pre-React static boot screen) also had its own independent `@media (prefers-color-scheme: dark)` block, removed the same day for the same reason - re-adding *only* that media query without also re-adding the `.dark`-class toggle would recreate the exact bug this whole investigation started from: a boot screen that matches system dark mode, flashing to a permanently-light app the instant React mounts.
- **If asked to "fix dark mode" or "why doesn't dark mode work" again**: this is not a bug to silently fix - explain that it was deliberately disabled at the user's own request and confirm before re-enabling anything. Re-enabling means restoring the inline `index.html` bootstrap script AND the `main.tsx` `matchMedia` listener together, not just one half.
...
- **One Loading Spinner Identity, App-Wide (HARD RULE, added 28 Aug 2026 after repeat complaints)** - the user has flagged "why are there 2 different loaders" multiple times across separate sessions; each time only part of the mismatch got fixed, so it kept resurfacing. There must be exactly ONE loading-spinner visual identity used everywhere in this app - a plain CSS border-ring (`border-[3px]`, `border-blue-100 border-t-blue-500` light / `border-slate-800 border-t-blue-400` dark, `rounded-full`), spun via the shared `.loading-screen-spinner-spin` class in `src/custom.css` (0.8s linear - NOT Tailwind's default `animate-spin`, which is 1s and visibly changes speed at a handoff between two loaders using it). Same for the boot-screen logo: `index.html`'s inline `#initial-loader` and `src/components/LoadingScreen.tsx` render the exact same `app-icons/icon-source.png` file at the same 64x64 size with NO wrapping box (no background/border/shadow/padding - a "glass card" frame around it, however subtle, is a second distinct look) and the same pulse timing (`.loading-screen-logo-pulse` in `custom.css`, 2s ease-in-out, opacity 1↔0.6).
  - **Never reach for a Flowbite/Lucide "spinner" icon component for this** - Flowbite's icon set has no real "Spinner" glyph; a `Loader2`/similar icon component silently falls back to a completely different shape (a refresh/two-curved-arrows icon), which is exactly how this regressed before. The plain CSS border-ring is the only correct implementation.
  - **Two places can never literally share one `<style>` block** (`index.html`'s loader is inline CSS so it paints before any bundle loads; `LoadingScreen.tsx`'s is a bundled Tailwind/custom.css component) - so when editing one side's spinner or logo treatment, always open the other side too (`index.html`'s `#initial-loader` styles, `LoadingScreen.tsx`, and `App.tsx`'s `TabContentFallback`) and mirror the change by hand. A drift between them is exactly this bug.
  - Smaller in-page loading contexts (e.g. `TabContentFallback` for lazy-loaded tab content) may reasonably use a **smaller size** than the full-page boot spinner, but must keep the same border style, colors, and spin speed - never a different color scheme, border width, or icon-based spinner.
...
- **Calendar bar conventions for OTA bookings (added 28 Aug 2026, explicit request)** - applies to both `TodayOverview.tsx` (multi-room overview) and `OperationalDashboard.tsx` (per-room calendar); both must stay in sync since they render the same concepts:
  - **Unconverted OTA-blocked bars (a synced Airbnb/Booking.com hold not yet turned into a real booking) must be red and pulsing** (`bg-red-600 dark:bg-red-700 border-red-700/40` + `animate-pulse`) - not the old static dark/slate color - since these are exactly the ones that still need staff action (see "Unconverted OTA Booking Alerts" below); a calm dark color didn't convey urgency. The legend swatch for this state (`legend_ota_blocked`) gets the same red+pulse treatment.
  - **Reversed 7 Sep 2026, explicit request: a converted OTA booking's bar shows ONLY the guest's first name + the OTA logo icon - no source text in the bar.** The 28 Aug 2026 rule below this one originally required spelling out the source in parentheses (`Gaurav (Booking.com)`) specifically because "just the guest's name with an icon" wasn't enough - that reasoning is now superseded: the icon carries the source, the parenthetical text was judged redundant clutter on an already-compact bar. Read `guest.otaSourceLabel || guest.otaSource` via `getOtaIcon()` (`src/utils/otaIcons.ts`) for the icon only; do not reintroduce the text suffix on the bar itself. The full name **and** the OTA name + logo still belong in the popover (a "Source:" row next to Room/Dates) and in `BookingDetailsModal.tsx`'s header badge - both already show it, that's where the detail lives, not on the compact bar.
  - **Every guest name shown on a compact calendar bar chip (direct or converted-OTA) shows first name only** - e.g. "Rohit Jain" renders as "Rohit". Use the shared `getFirstName()` helper in `src/utils/nameUtils.ts` (already existed, unused until this) rather than a fresh `.split(' ')[0]` inline each time. Deliberately scoped to the compact bar labels only - full names stay untouched in the hover/click detail popovers, the "+N more" overflow list, and any modal (ID verification, WhatsApp share, etc. all need the real full name).
  - **The legend entry for a converted OTA booking must read "Converted OTA Bookings"** (i18n key `legend_ota_converted` in `en.ts`), not the old "OTA Booking (Airbnb / Booking.com)" wording - it was ambiguous next to the "OTA-Blocked" entry about which one meant "already a real booking."
...
- **MANDATORY: a "greyed-out because a required field is missing" button must still be clickable, not natively `disabled` (added 28 Aug 2026, explicit request + screenshot: "if I click on greyed out buttons... it should show error/warning on the form fields")**: a real HTML `disabled` button swallows clicks entirely, so the user gets zero feedback about *why* nothing happened - they can't tell "form incomplete" from "button is broken." Fix: don't pass `disabled` for the validation-gate case - keep the greyed-out look via `className="opacity-50"` (or equivalent) instead, and let the click reach a handler that sets a "the user has now attempted to proceed" flag (gating the live field errors above so they don't show on a pristine, never-touched form) and, for any blocking reason that isn't a plain text `Input` with its own `error` slot (a toggle-button pair, a checkbox, a cross-field rule), shows a toast explaining it instead. `disabled` is still correct and should stay for a genuinely-in-flight action (e.g. `loading` during a real submit) - only the "form is incomplete" case needs this treatment. First applied in `SelfOnboardingWizard.tsx`'s "Next Step"/"Complete Setup & Launch" buttons - apply the same pattern to any other step-gated wizard button found to have the same silent-disabled behavior.
...
### Channel Manager / Channex — Connect-Channel Protocol (added 3 Sep 2026, after a real live incident)

> **📖 MANDATORY: read `CHANNEX.md` (repo root) before doing ANY Channex work** - connecting or removing a channel, pushing ARI, touching `php/channex/`, importing from an OTA, debugging a sync, or answering a question about how the integration behaves. It is the single source of truth: architecture and file map, the verified API facts (so they aren't re-derived), Airbnb's read/write surface, the hard safety rules, and the debugging playbook. The section below is the summary of one incident; `CHANNEX.md` is the full picture and is kept current - correct it there, with the date, when something proves wrong.

Found live 3 Sep 2026 on Patel Colony (a MULTI_KEY property, 7 rooms): "Go Live" on its Airbnb + Booking.com connections reported success, but Channex's own Inventory tab showed **AVL=0 for every room, every date** - both live channels were silently blocking all bookings from the day the property went live. Root cause and fix below; follow this checklist for every future channel connection, not just Airbnb.

- **A "whole property" ARI push must never enqueue with a bare `room_id=null`.** `channex_mappings` is per-room for a MULTI_KEY property (no `room_id IS NULL` row exists once it has real `MULTI_KEY_ROOM` children) - `ChannexAdapter::getMapping()` finds nothing for `(propertyId, null)`, `pushAvailability()`/`pushRestrictions()` return `success:false`, and `AriDrainWorker::processBatch()` just marks the outbox rows failed and moves on **silently** - neither `channex_channel_activate`'s pre-activation push nor the manual `channex_push_ari` action checked that result, so activation proceeded and reported success while nothing was actually pushed for any of the property's rooms. Fixed via `getChannexPushRoomIds($pdo, $propertyId)` (`php/channex/outbox.php`) - resolves real `MULTI_KEY_ROOM` children and pushes each individually, falls back to `[null]` unchanged for a single-unit property. **Any new code that does a property-wide ARI push must call this helper first, never hardcode `room_id=null`.**
- **After any "Go Live"/activate action, verify on Channex's own Inventory tab (or a real `GET availability`/`GET tasks/:id` check) that AVL actually shows open** - don't trust a "success" toast alone. A push can fail server-side (wrong mapping, transient network) without the caller ever surfacing it, exactly as happened here.
- **Before connecting any channel for a property that may already have existing reservations on that OTA, pull them in - don't assume they'll show up on their own.** `booking_revisions/feed` only ever holds *unacknowledged* events; it does **not** contain a listing's pre-existing reservations unless something explicitly told Channex to go fetch them first (confirmed empty on Patel Colony despite real, confirmed Airbnb bookings that predated the connection). For Airbnb specifically, call `ChannexChannelClient::loadFutureReservations($channelId)` (`POST channels/:id/execute/load_future_reservations`, documented at `https://docs.channex.io/channel-api-examples/airbnb.md`) - it triggers Channex to fetch the listing's existing reservations from Airbnb, which then arrive through the normal webhook pipeline within seconds. Other channel types may need a different mechanism - check that channel's own Channex docs before assuming none exists; don't declare "not possible" without checking.
- **MANDATORY, not just "good practice": before running ANY broad "open every date" availability push (a correction, a re-sync, anything covering a wide range), explicitly ask the user whether they have manually blocked any dates directly on that OTA's own calendar - do not run the push first and sort it out after.** Ground Code only knows about (a) its own `guests` bookings and (b) blocks set *inside* Ground Code (`room_rate_rules.stop_sell`) - it has **zero** visibility into a block the host set directly on the OTA's own calendar, and a wide push will silently reopen any such block because nothing in Ground Code's data says it should stay closed. **This actually happened** (3 Sep 2026, this same incident): a corrective push was run without asking first, and it silently reopened manually-blocked dates on Airbnb. The only reason it was recoverable at all was that the user happened to have left host notes on those specific blocked dates, which let them tell "I blocked this on purpose" apart from "the bug closed this by default" after the fact - **notes are not a safeguard to rely on, they are not guaranteed to exist, and their absence would have made this unrecoverable** (no record of which dates, no way to know what was even lost). Asking the question up front costs one message; reconstructing lost intent after an irreversible push may not be possible at all. If the user does have manual blocks, get the specific dates/rooms and record them in Ground Code (Pricing & Rates → block) *before* running the push, so the block round-trips correctly to every channel afterward instead of only ever existing on the one OTA.
- **The "Remove" button on a channel connection (`ChannelConnectionsPage.tsx`) is real and irreversible** - it deletes the actual Channex channel object and every room mapping on Channex's side, not just local state (confirmed live: a working, fully-mapped Airbnb connection was deleted this way and had to be fully re-authorized and re-mapped from scratch). Its confirm dialog does warn about this, but don't click through it casually while testing/debugging a connection that's actually working.
- **This is a SaaS product for many tenants - the fix for a risk found on one property is a UI-level safeguard every tenant sees, not a special-cased pause for that one property, and not a global feature disable either.** When the rate-fallback risk (see next bullet) was found, the first instinct was to hard-disable rate pushing app-wide - wrong call, since that would silently break the core channel-manager feature for every other tenant who doesn't have this specific problem. The actual fix: an explicit, mandatory confirmation the owner has to consciously accept before the risky action runs, enforced both client-side (so they see it) AND server-side (so it can't be silently bypassed the way `confirmed_existing_bookings` briefly was - see the "checkbox existed but its value was never sent" bug earlier in this same incident). Apply this same shape - informed-consent gate, not silent pause or silent global change - to the next risk like this that turns up.
- **Rate-fallback risk, same shape as the availability one**: `AriDrainWorker::computeCompressedRestrictions()` falls back to the property's flat `default_tariff` for any date with no explicit `room_rate_rules` row, and pushes that as the real price. A property with no rates entered yet (e.g. freshly connected rooms) will push the flat default over its *entire* pushed range, which can silently overwrite different pricing already set directly on the OTA - the exact pricing equivalent of the availability-block problem above. Gated the same way: `ChannelConnectWizard.tsx`'s Step 4 has a second required checkbox (`confirmedRateFallback`) alongside the bookings one, enforced server-side too (`channex_channel_activate` 422s without `confirmed_rate_fallback`); the manual "Push Availability, Rates & Restrictions" button (`ChannelManager.tsx`) has its `confirm()` dialog message spelling out both risks (rate fallback AND availability reopen) before every push, not just Go Live's one-time gate. Any new code path that can push a wide-range rate or availability update needs the same explicit, spelled-out consent gate - don't add a new push path without one.
- **A confirm() dialog only covers actions taken through the app - it does nothing for a corrective/fix push the agent runs directly against the server** (a one-off script, exactly like this incident's own `manual_correction_push`). Found again 4 Sep 2026: a corrective rate push (legitimate, needed to fix the room-enumeration bug above) reached Airbnb with no in-app prompt at all, because a server-side script never goes through `ChannelManager.tsx`'s dialog - the user found out only by asking "how come my rates are pushed" after noticing it independently. Two separate fixes now, both required, neither a substitute for the other: (1) **the agent must explicitly ask the user before running ANY corrective/fix push itself, every single time - reporting it after the fact is not the same thing and is not sufficient**, this was already the rule above and evidently needs restating; (2) as a guaranteed backstop that doesn't depend on anyone remembering rule (1), every rate/restriction push that actually completes - regardless of what triggered it - is now recorded via `recordRatePushAlert()` (`php/channex/outbox.php`), called from the one chokepoint every push of any kind has to pass through (`AriDrainWorker::processBatch()`, right after a `kind='rates'` group succeeds). The app prompts the user about it via a real modal (`window.showAlert()`, `Header.tsx`'s mount-time check against `get_pending_rate_push_alerts`) the next time they're in Ground Code, not a passive log page they'd have to remember to check. Any future code path that can cause a rate/restriction push doesn't need its own copy of this - it already runs through `AriDrainWorker::processBatch()` and is covered automatically.
- **SUPERSEDED 9 Sep 2026 — the two consent checkboxes (`confirmed_existing_bookings` / `confirmed_rate_fallback`) described in the "SaaS product for many tenants" and "Rate-fallback risk" bullets above no longer exist, and neither does `ChannelManager.tsx`'s `confirm()` dialog named in the bullet directly above.** They were replaced by the **Push Confirmation Gate** (`src/components/PushConfirmationGate.tsx` + `php/channex/push_preflight.php`, full spec in CHANNEX.md §5.4b), which both wide-push paths now open: `channex_channel_activate` (Go Live) and `channex_push_ari` (the manual push, whose `confirm()` dialog is also gone). Three steps: (1) the actual nights with no explicit price, and the exact default rate that would be sent for them — computed, not asserted; (2) the actual dates about to be marked bookable, because Ground Code cannot see a host's manual OTA blocks and only they can catch that; (3) the **property's name typed out** — not a fixed word like "PUSH", since a fixed word becomes muscle memory and pushing to the wrong property is as damaging as pushing wrong values. Still enforced server-side (`requireChannexPushConfirmation()`), for exactly the reason the checkboxes were. The checkboxes were **replaced, not supplemented** — three confirmations in a row train people to click through all three, including the one that matters. The principle in the bullets above (informed-consent gate, enforced both sides, never a silent global disable) is unchanged; only the mechanism moved.
- **An OTA import must never push and never activate (HARD RULE, 9 Sep 2026, explicit instruction: "no matter what ... no availability is pushed from app end but only imported").** `autoProvisionPropertyFromAirbnb()` imports content, maps rooms, pulls existing reservations, parks the connection at `ready_to_activate`, and stops — it does not enqueue ARI and does not call `activateChannel()`. Activating **is** a write: Channex documents it as "a full synchronisation pushes availability, rates and restrictions", so there is no "activate but push nothing" — pushing our view reopens dates blocked directly on the OTA, and pushing nothing leaves AVL=0 everywhere (the 3 Sep Patel Colony incident). Between 8 and 9 Sep 2026 this path did activate; that is why it briefly required the consent flags. Going live is now a separate, deliberate step through the gate above. **Do not re-add an activate or `enqueueOutboxItem()` call to any import path.** See CHANNEX.md §5.4a.
- **Airbnb's sync categories cannot solve the rate-overwrite risk — verified live 9 Sep 2026** (CHANNEX.md §6, "Sync category"). Airbnb offers exactly two settings for API-connected listings and **neither** lets the software manage availability while the host keeps nightly pricing. So this is a process problem, not a settings problem — don't go looking for a platform switch. Separately: 7 of 10 connected listings are `sync_all`, under which Airbnb greys out photos/content and expects the software to supply them, while we deliberately never import photos (CHANNEX.md §8.6) — whether that leaves a host unable to edit their own listing content anywhere is **not yet established** and is worth settling before more listings go live.

### OTA Import Must Never Rewrite a Property's Identity (HARD RULE, added 6 Sep 2026 after a live incident)

**An OTA listing import must NEVER rename a MULTI_KEY parent property, under any circumstance - not as a default, not as an opt-in, not via a hand-crafted API call.**

**What happened**: importing an Airbnb listing onto Patel Colony (a MULTI_KEY parent with 7 rooms) renamed the whole property to that one listing's title - "Guest suite in Jaipur". It was found on the **public booking engine** (`staging.ground-code.com/patel-colony/#book`), where it rendered as the heading above all 7 rooms, to guests. `OtaPropertyImporterModal.tsx` pre-ticked "Property name" for every target, and `PropertyImporter::applyToProperty()` wrote whatever it was handed.

**Why it can never be right**: an OTA listing describes exactly ONE sellable unit. A MULTI_KEY parent is the *building* that contains it. There is no version of "the listing's title" that is a correct name for the parent, so this isn't a field to default off - it's a write to refuse.

**How it's enforced now** (both halves are required; neither is sufficient alone):
- **Server-side, the actual rail** - `applyToProperty()` in `php/api/property_importer.php` resolves `$isMultiKeyParent` (from `property_type === 'MULTI_KEY'`, OR any non-deleted row with `parent_property_id = <target>` - a property carrying real child rooms is a parent whatever its type column says) and refuses the `name` write outright, returning it in the response's `blocked_fields` so the caller can say what it declined instead of silently dropping it.
- **`name` is excluded from the endpoint's "apply everything" default.** `$applyAll = empty($selectedFields)` still governs every other field, but a rename only ever happens when a caller names `name` explicitly. An omitted/empty `selected_fields` must never be able to rename a property.
- **Client-side** - the "Property Name" checkbox isn't rendered at all when the target is MULTI_KEY (`propertyType` is threaded in from `EditPropertyPage.tsx`), so nobody ticks something that can't be applied.
...
- Standalone, DB-independent error console at `php/errors/` (visit `/php/errors/` directly). Reads/writes `php/errors/logs.json` via `TelescopeLogger` (`php/errors/logger.php`), portals: `requests`/`php`/`sql`/`js`/`telegram`/`whatsapp`/`security`/`404`/`ai_chat`, plus `staff_activity`/`login` (synthesized client-side from `get_audit_logs`, a different data source).
- **`ai_chat` portal (added 27 Aug 2026)**: both `php/api/ai_assistant.php`'s log calls (`AI Query`/`AI Outcome`/`Gemini Call Failed`/`OpenAI Call Failed`/`OpenCode Zen Call Failed`) and `php/api/ai_config.php`'s (`AI Config Updated`, moved here from the never-counted, never-tabbed `system` portal) live here, with a dedicated "AI Assistant" sidebar tab in `index.php`. **This whole portal never sends a Web Push phone alert** (`logger.php`'s `maybeSendWebPushAlert()` returns early on `$portal === 'ai_chat'`, before the general severity denylist) - explicit request: the user is fine with these being logged, just doesn't want a phone notification for ordinary AI chat traffic, including provider-call failures (a narrower severity-only denylist existed briefly first and still let call-failure severities push through - replaced with the portal-wide gate since that undershot the actual ask). Adding a new portal means touching 4 places, not just `logger.php`'s counts array - `index.php` hardcodes the same portal-name list twice more in plain JS (`getUnseenCounts()`'s `counts` object and the server-count-fallback `counts` object in `loadPortalLogs()`) for badge totals; miss one and that portal's sidebar badge silently stays stuck at 0 even though entries are logging correctly.
...
### Page & Card Padding Standard (added 27 Aug 2026, full-app sweep)

- **Page-edge padding**: `px-4 sm:px-6 lg:px-8` - the shared `<main>` in `App.tsx` (every tab inside a property: Dashboard/Bookings/Team/Kitchen/Edit Property/etc.) and `TenantDashboard.tsx`'s own `<main>` (a separate top-level page, not one of those tabs) both use this now. Before this sweep they'd drifted independently - the shared `<main>` was `px-1` (4px) on mobile, and `TenantDashboard.tsx` had its own unrelated flat `px-6` - found via a user report that different pages "felt" inconsistently inset on a phone screen, confirmed by actually diffing the two files' container classes.
- **Section-card padding**: `p-4 sm:p-6` - one/few-per-page content cards (an Alerts card, a per-guest booking card, a per-staff-member card, `EditPropertyPage`'s form card) all use this. **Deliberately NOT applied to dense repeating list items** (KDS kitchen ticket cards, Take Order menu rows, table rows) - those are intentionally compact so more of them fit on screen at once; blowing them up to match section-card padding would hurt scanability for someone working a busy kitchen queue, not fix an inconsistency.
- **flowbite-react `<Card>`'s padding is NOT settable via its own `className` prop** - confirmed in `node_modules/flowbite-react/dist/components/Card/{Card,theme.js}`: the actual padding (`theme.root.children = "... p-6"`) lives on an inner div built separately from the outer wrapper `className` merges onto - the exact same "className reaches the wrong DOM node" class of bug already found this same day in `Input.tsx`'s `TextInput` usage and `DashboardFooter.tsx`'s `FooterLink`. Since ~15 files use `<Card>` directly with no local override, this was fixed once, globally, via a `<ThemeProvider theme={{ card: { root: { children: 'flex h-full flex-col justify-center gap-4 p-4 sm:p-6' } } }}>` wrapping `<App />` in `main.tsx` - not by patching each call site (or relying on every future one to remember to).
- If you add a new top-level page or section card, use these two scales rather than picking a fresh padding value - that's exactly how this drifted in the first place.
