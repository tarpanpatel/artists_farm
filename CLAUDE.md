...
### configuration.php on Staging — Same Pattern, Fixed 26 Aug 2026
- The pattern above repeated on a second file: `php/api/configuration.php` (backs `get_system_settings`, `get_telegram_templates`, `get_ui_configuration`, icon libraries, etc.) has its own missing-file self-heal in `router.php`, added 23 Aug 2026 on the (never confirmed via CPGuard's scan logs, just presumed by analogy) theory that the same scanner also quarantines this file. Found live 26 Aug 2026: `get_system_settings`/Telegram Notifications 503ing on staging with `"Configuration module unavailable"`.
- **Two bugs, both now fixed**: (1) the self-heal still *copied* production's bytes onto staging's own path — the exact losing cycle the telegram.php fix above already ruled out; (2) its fallback path was still the pre-migration `/home/apartment/public_html/php/api/configuration.php` — dead since the 25 Aug 2026 cutover (see above), so even the copy-back attempt was reading from a stale, no-longer-deployed location. Now `require_once`s straight from `/home/apartment/ground-code.com/php/api/configuration.php` when staging's own copy is missing, same as telegram.php.
- **Why requiring directly is safe here** (worth checking per-file, not assumed automatically safe): `configuration.php` itself does `require_once __DIR__.'/../config/database.php'`, which would otherwise pull in **production's** copy of `database.php` from a foreign `__DIR__` — but unlike `module_manager.php` (plain function definitions, needed a `function_exists()` guard), `database.php` is a stateful bootstrap file (CORS headers, the CSRF origin check, opening `$pdo`) that already double-guards every `define()` it makes (`if (!defined(...))`) in anticipation of exactly this kind of cross-environment re-require. `configuration.php`'s own require is now additionally guarded with `if (!defined('APP_IS_STAGING_ENV'))` (a symbol only `database.php` defines) so it skips the wasted second `$pdo` connection and duplicate `header()` calls entirely when it's already loaded — belt-and-suspenders on top of `database.php`'s own idempotency, not strictly required to avoid a fatal.
- If a third file ever needs this same self-heal: check whether it internally requires anything stateful (not just function definitions) before assuming "require directly from the whitelisted path" is a safe drop-in — verify the transitively-required file's own re-require behavior first, the way this entry and the telegram.php one above both had to.
