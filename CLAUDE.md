...
- **Never push a broad "open every date" availability correction without warning the user first.** Ground Code only knows about (a) its own `guests` bookings and (b) blocks set *inside* Ground Code (`room_rate_rules.stop_sell`) - it has **zero** visibility into a block the host set directly on the OTA's own calendar. A wide corrective push (e.g. re-running the fix above) will silently reopen any such manual block, because nothing in Ground Code's data says it should stay closed. If the user has ever manually blocked dates directly on an OTA, get that list from them and record it in Ground Code (Pricing & Rates → block) *before* running any full-range push, so it round-trips correctly to every channel afterward instead of only existing on the one OTA.
