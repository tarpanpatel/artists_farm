...
| Source | Data |
|---|---|
| `GET /channels/:id/action/listings` | One call, whole account. **Exactly eight fields, verified live 9 Sep 2026** — `id`, `type`, `title`, `city`, `quality_status`, `country_code`, `occupancies` (valid guest counts, max = capacity), `synchronization_category`. **No street address and no coordinates**: `city` is the ONLY location it carries, so it cannot tell two buildings in one city apart (see the location note below). **`title` is COMPOSED, not the raw Airbnb listing name** (verified live 12 Sep 2026): it joins the parent listing title and the unit title with a middle dot (`·`). On this account every unit title already begins with the parent's, so 5 of Patel Colony's 7 rooms imported as doubled names — `The Designer's Studio · The Designer's Studio ★Central Area★`, visible to staff on the dashboard calendar. Always pass a listing title through `normalizeOtaListingTitle()` (`php/channex/ota_provisioner.php`) before writing it to a room; it strips a leading segment only when that segment is a case-insensitive prefix of what follows, so a genuine `A · B` survives intact. |
