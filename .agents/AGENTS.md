...
- **Unified Component & Single Source of Truth Rule (DRY with Conditional Variations)**: Past, present, and future coding must strictly follow the Single Source of Truth principle: never build or maintain duplicate or parallel components, forms, drawers, or dialogs for different contexts (e.g., Single-Key vs. Multi-Key, Live Checkout vs. Past Bill Audit). Always share the exact same component code, using clean conditional props (`isMultiKey?: boolean`, `mode?: 'checkout' | 'audit-modify'`). Any styling, design token, typography, or element update must be made once in the shared component, instantly and automatically reflecting across all variants without code drift (unless explicitly instructed to be conditional to one place only).
- **No Border-Radius on Edge-to-Edge Mobile Blocks Rule**: Any block, card, dashed empty-state container, table wrapper, or section row that spans the full viewport width on mobile (`w-full` where `.app-shell__main` has `px-0` on `< sm` screens) must strictly use `rounded-none sm:rounded-lg`. Never allow rounded corners on full-bleed mobile blocks, as curved corners create visible gaps/notches against the straight screen edges. Additionally, remove left and right side borders on full-bleed mobile elements (`border-x-0 sm:border-x` or `border-y sm:border`).
...
- **Date Range in Active Date-Range Rules Rule (DD/MM/YY)**: In all pricing, seasonal rates, and restriction rule tables/lists (such as Active Date-Range Rules), date ranges must strictly be rendered in 2-digit day/month/year format (`dd/mm/yy` e.g. "09/09/26 → 15/09/26") using `formatDateDDMMYY()`, never as raw ISO `YYYY-MM-DD` strings.
