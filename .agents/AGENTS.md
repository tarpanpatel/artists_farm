...
- **No Generic Tooltips Rule**: Never use generic OS/browser `title="..."` attributes for UI tooltips or hover cards. Everywhere across the site, interactive hover information and tooltips must use Flowbite Popover / Tooltip styling following [Flowbite Popover](https://github.com/themesberg/flowbite/blob/main/content/components/popover.md) with structured header/body styling, dark mode tokens (`dark:bg-gray-800 dark:border-gray-700`), `rounded-lg`, and `shadow-lg`.
