...
try {
    ...
    if (!$stmt->fetch()) {
        ...
        echo "âœ“ Added tenant_id to properties table\n";
    } else {
        echo "âœ“ tenant_id already exists\n";
    }
} catch (Exception $e) {
    echo "âœ— Error: " . $e->getMessage() . "\n";
}
...
try {
    ...
    echo "âœ“ Created property_modules table\n";
} catch (Exception $e) {
    echo "âœ— Error: " . $e->getMessage() . "\n";
}
...
try {
    ...
    echo "âœ“ Created system_modules table\n";
...
try {
    ...
    if (!$stmt->fetch()) {
        ...
        if ($stmt_old_pass->fetch()) {
            ...
            echo "âœ“ Added password_hash column to users table\n";
            ...
            echo "âœ“ Migrated existing passwords to password_hash\n";
        } else {
            $pdo->exec("ALTER TABLE users ADD COLUMN `password_hash` VARCHAR(255) NOT NULL"); // If no old password, make it NOT NULL
            echo "âœ“ Added password_hash column to users table (no old 'password' column found)\n";
        }
    }
...
echo "âš ï¸ Platform admin must be created via manual installation process, NOT auto-generated\n";
echo "âœ“ Skipped auto-creation to prevent hardcoded credentials\n";
...
try {
    ...
    echo "âœ“ Created default tenant 'Ground Code Platform'\n";
...
echo "\nNew Structure:\n";
echo "1. Platform Admin (You) â†’ Manages Tenants\n";
echo "2. Tenants (Restaurant Chains) â†’ Can have multiple Properties\n";
echo "3. Properties (Locations) â†’ Can enable/disable Modules\n";
echo "4. Modules (Kitchen, Billing, etc.) â†’ Assigned per Property\n";
echo "\nNext steps:\n";
