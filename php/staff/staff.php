...
function handleStaffRequests($pdo, $request_method, $action) {
    ...
    try {
        ...
        // Seed all staff if table is empty - single source of truth for all 11 staff
        $check = $pdo->query("SELECT COUNT(*) FROM staff_users")->fetchColumn();
        if ($check == 0) {
            $seedUsers = [
            ...
            $stmt = $pdo->prepare("INSERT INTO staff_users (id, username, full_name, role, phone, monthly_salary, status, is_financial_handler, passcode, qr_code_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            foreach ($seedUsers as $u) {
                $stmt->execute($u);
            }
        }

        // Seed default initial payees if empty
        $checkPayees = $pdo->query("SELECT COUNT(*) FROM payee_entities")->fetchColumn();
        if ($checkPayees == 0) {
            $seedPayees = [
            ...
            $stmt = $pdo->prepare("INSERT INTO payee_entities (id, name, type, qr_code_url) VALUES (?, ?, ?, ?)");
    ...
    switch ($action) {
        ...
        case 'get_users':
            try {
                $stmt = $pdo->query("SELECT id, username, full_name as fullName, role, phone, monthly_salary as monthlySalary, status, is_financial_handler as isFinancialHandler, passcode, qr_code_url as qrCodeUrl FROM staff_users ORDER BY CAST(id AS UNSIGNED) ASC, id ASC");
        ...
        case 'add_user':
            if ($request_method === 'POST') {
                ...
                try {
                    $stmt = $pdo->prepare("INSERT INTO staff_users (id, username, full_name, role, phone, monthly_salary, status, is_financial_handler, passcode, qr_code_url)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        case 'update_user':
            if ($request_method === 'POST') {
                ...
                try {
                    ...
                    $existing = null;
                    if (!empty($input['id'])) {
                        $stmtSel = $pdo->prepare("SELECT * FROM staff_users WHERE id = ?");
                        $stmtSel->execute([$input['id']]);
        ...
        case 'get_payees':
            try {
                $stmt = $pdo->query("SELECT id, name, type, qr_code_url as qrCodeUrl FROM payee_entities ORDER BY name ASC");
                $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        case 'add_payee':
            if ($request_method === 'POST') {
                $input = json_decode(file_get_contents('php://input'), true);
                try {
                    $stmt = $pdo->prepare("INSERT INTO payee_entities (id, name, type, qr_code_url) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE name = VALUES(name), type = VALUES(type), qr_code_url = VALUES(qr_code_url)");
                    $stmt->execute([
                        $input['id'] ?? ('pay-' . time()),
                        $input['name'],
                        $input['type'] ?? 'Vendor',
        ...
        case 'delete_payee':
            if ($request_method === 'POST') {
                $input = json_decode(file_get_contents('php://input'), true);
                try {
                    $stmt = $pdo->prepare("DELETE FROM payee_entities WHERE id = ?");
                    $stmt->execute([$input['id']]);
        ...
        case 'get_attendance':
            $count = 0;
            try { $count = $pdo->query("SELECT COUNT(*) FROM staff_attendance")->fetchColumn(); } catch (PDOException $e) {}
            if ($count == 0) {
                $seedAttendance = [
                ...
                $stmt = $pdo->prepare("INSERT INTO staff_attendance (attendance_date, user_id, status) VALUES (?, ?, ?)");
                foreach ($seedAttendance as $a) {
                    try { $stmt->execute($a); } catch (PDOException $e) {}
                }
            }
            try {
